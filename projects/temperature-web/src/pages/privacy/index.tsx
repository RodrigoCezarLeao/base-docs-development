...
export default function PrivacyPage() {
  ...
  const withdraw = () => {
    recordConsent.mutate('withdrawn', {
      onSuccess: () => toast.success(t('privacy.withdrawn')),
  ...
  const deleteData = () => {
    deleteMyData.mutate(undefined, {
      onSuccess: (res) => toast.success(t('privacy.deleted', { count: res.data ?? 0 })),
      onError: () => toast.error(t('common.error')),
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <Button variant="secondary" onClick={() => navigate('/')}>{t('privacy.back')}</Button>
      </header>

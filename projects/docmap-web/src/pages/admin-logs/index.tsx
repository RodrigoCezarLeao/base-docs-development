...
export default function AdminLogsPage() {
  ...
  // Default to the newest available day once the file list arrives.
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">{t('logs.title')}</h1>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6">
        ...
        <div className="flex flex-wrap items-end gap-3 mb-4">
          <label className="flex flex-col gap-1 text-sm">
            <span className="font-medium text-gray-700">{t('logs.date')}</span>
            <select
              value={date}
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="font-medium text-gray-700">{t('logs.level')}</span>
            <select
              value={level}
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              <tbody className="divide-y divide-gray-100">
                {data?.items.map((entry, i) => (
                  <tr key={`${entry.timestamp}-${i}`} className="align-top">
                    <td className="px-3 py-2 font-mono text-xs text-gray-500 whitespace-nowrap">{entry.timestamp}</td>
                    <td className="px-3 py-2">
                      <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${levelColor[entry.level] ?? 'text-gray-600 bg-gray-100'}`}>
                        {entry.level}
                      </span>
                    </td>
                    <td className="px-3 py-2 font-mono text-xs text-gray-600">{entry.category}</td>
                    <td className="px-3 py-2 text-xs text-gray-600">{entry.userId === '-' ? '' : entry.userId}</td>
                    <td className="px-3 py-2 text-gray-800 break-words">{entry.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {!isFetching && (data?.items.length ?? 0) === 0 && (
            <p className="text-center text-gray-500 py-8">{t('logs.empty')}</p>
          )}
        </div>
